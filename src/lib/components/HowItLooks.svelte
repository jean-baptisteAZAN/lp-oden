...
<h2 class="h2 text-center -mt-10 mb-20 md:-mt-20 md:mb-40">À quoi ça ressemble ?</h2>
...
<div class="flex flex-col xl:flex-row px-8 md:px-20 xl:px-40 w-full">
	...
	<div class="md:hidden flex flex-row items-center justify-center mt-10">
		<button
			on:click={() => rotateToIndex(currentIndex + 1)}
			class="btn variant-filled w-80 animate-bounce"
		>
			{wording[(currentIndex + 1) % wording.length].title}
		</button>
	</div>
	<div
		class="hidden md:flex flex-col items-center justify-center w-full xl:w-1/2 lg:mt-20 2xl:mt-0"
	>
